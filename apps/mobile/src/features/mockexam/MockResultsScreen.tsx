...
export function MockResultsScreen({
  navigation,
  route,
}: NativeStackScreenProps<RootStackParamList, "MockResults">) {
  const identity = useAppStore((s) => s.learnerIdentity);
  const query = useQuery({
    queryKey: ["mock-results", route.params.attemptId],
    queryFn: () => learningApi.mockResults(identity, route.params.attemptId),
  });
  ...
  return (
    <Screen>
      <MockResultView result={query.data} history={history.data} />
      <Button label="Return home" onPress={() => navigation.popTo("Home")} />
      <Button
        label="Review answers"
        variant="secondary"
        onPress={() =>
          navigation.navigate("MockAnswerReview", {
            attemptId: route.params.attemptId,
          })
        }
      />
      <Button
        label="Retake mock exam"
        variant="text"
