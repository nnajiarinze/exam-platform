...
export function MockQuestionScreen({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'MockQuestion'>) {
  ...
  useEffect(() => setSelected(question.data?.selectedOptionIds ?? []), [question.data?.attemptQuestionId, question.data?.selectedOptionIds]);
  const remaining = useCountdown(question.data?.remainingSeconds ?? attempt.data?.remainingSeconds ?? -1);
  const answer = useMutation({ mutationFn: (optionIds: string[]) => learningApi.answerMockQuestion(identity, attemptId, question.data!.attemptQuestionId, optionIds, question.data!.answerVersion), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['mock-attempt', attemptId] }); queryClient.invalidateQueries({ queryKey: ['mock-question', attemptId, sequenceNumber] }); }, onError: async () => { const refreshed = await question.refetch(); setSelected(refreshed.data?.selectedOptionIds ?? []); } });
  ...
  return <Screen><View style={styles.header}><Text style={styles.progress}>Question {sequenceNumber} of {question.data.totalQuestions}</Text><Text accessibilityLabel={`${displayedRemaining} seconds remaining`} style={styles.timer}>{formatCountdown(displayedRemaining)}</Text></View>
    ...
    {question.data.questionType === 'MULTIPLE_CHOICE' && <Text style={styles.hint}>Select all answers that apply.</Text>}
    {question.data.answerOptions.map((option) => <Pressable accessibilityRole={question.data.questionType === 'MULTIPLE_CHOICE' ? 'checkbox' : 'radio'} accessibilityState={{ checked: selected.includes(option.id), disabled: answer.isPending }} disabled={answer.isPending} key={option.id} onPress={() => setSelected((current) => question.data.questionType === 'MULTIPLE_CHOICE' ? (current.includes(option.id) ? current.filter((id) => id !== option.id) : [...current, option.id]) : [option.id])} style={[styles.option, selected.includes(option.id) && styles.selected]}><Text style={styles.optionText}>{option.text}</Text></Pressable>)}
    <Button label={answer.isPending ? 'Saving answer…' : 'Save answer'} disabled={!selected.length || answer.isPending} onPress={() => answer.mutate(selected)} />
...
const styles = StyleSheet.create({ header: { flexDirection: 'row', justifyContent: 'space-between' }, progress: { color: theme.colors.muted }, timer: { color: theme.colors.error, fontSize: 18, fontWeight: '700' }, prompt: { fontSize: 22, fontWeight: '700', lineHeight: 30 }, hint: { color: theme.colors.muted }, option: { backgroundColor: theme.colors.surface, borderColor: theme.colors.border, borderRadius: 10, borderWidth: 1, padding: 14 }, selected: { borderColor: theme.colors.primary, borderWidth: 2 }, optionText: { fontSize: 17 }, actions: { flexDirection: 'row', gap: 8 } });
