...
export function QuestionCard({ question, category, selectedId, result, submitting = false, onSelect }: { question: PracticeQuestion; category?: string; selectedId?: string; result?: AnswerResult; submitting?: boolean; onSelect: (id: string) => void }) {
  ...
  return <View style={styles.container}>
    ...
    <View accessibilityRole="radiogroup" style={styles.options}>{question.answerOptions.map((option, index) => {
      const selected = selectedId === option.id;
      const correct = Boolean(result) && option.id === result?.correctAnswerOptionId;
      const incorrect = Boolean(result) && selected && !result?.correct;
      return <Pressable accessibilityRole="radio" accessibilityState={{ checked: selected, disabled: Boolean(result) || submitting }} disabled={Boolean(result) || submitting} key={option.id} onPress={() => onSelect(option.id)} style={({ pressed }) => [styles.option, selected && styles.selected, correct && styles.correct, incorrect && styles.incorrect, pressed && styles.pressed]}>
