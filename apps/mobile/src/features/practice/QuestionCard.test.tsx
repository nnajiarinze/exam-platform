...
const question = {
  ...
  questionId: "q1",
  prompt: "Who makes Swedish laws?",
  questionType: "SINGLE_CHOICE" as const,
  answerOptions: [
    { id: "a1", text: "Riksdagen" },
    { id: "a2", text: "Polisen" },
  ],
  selectedOptionIds: [],
  sequenceNumber: 1,
...
it("renders and selects a question option", async () => {
  const onSelect = jest.fn();
  const view = await render(
    <QuestionCard question={question} selectedIds={[]} onSelect={onSelect} />,
  );
  fireEvent.press(view.getByText("Riksdagen"));
  expect(onSelect).toHaveBeenCalledWith("a1");
});
it("shows server answer feedback", async () => {
  const view = await render(
    <QuestionCard
      question={question}
      selectedIds={["a1"]}
      ...
      result={{
        ...
        correctOptionIds: ["a1"],
        explanation: "Parliament decides.",
        optionFeedback: [],
        sessionProgress: { answered: 1, total: 3 },
      }}
    />,
  );
  expect(view.getByText("Correct")).toBeTruthy();
  expect(view.getByText("Parliament decides.")).toBeTruthy();
});
it("does not reveal feedback before submission", async () => {
  ...
  expect(initial.queryByText("Study tip")).toBeNull();
});
  ...
  const answered = await render(
    <QuestionCard
      ...
      result={{
        ...
        correctOptionIds: ["a1"],
        explanation: "Riksdagen makes laws.",
        optionFeedback: [],
        sessionProgress: { answered: 1, total: 3 },
      }}
    />,
  );
  expect(answered.getByText("Study tip")).toBeTruthy();
