...
Configure `EXPO_PUBLIC_LEARNING_BASE_URL`, `EXPO_PUBLIC_EXAM_ID`, and the development learner identity described in the root environment example. Run `npm start` for Expo, `npm test` for component tests, and `npm run generate:api` after changing the Learning Service OpenAPI contract.
