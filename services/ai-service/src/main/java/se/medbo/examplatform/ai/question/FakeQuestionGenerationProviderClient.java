...
final class FakeQuestionGenerationProviderClient implements QuestionGenerationProviderClient {
  ...
  private Proposal proposal(Request r,String type,int index){
    ...
    var evidence=new ArrayList<SourceEvidence>();if(!r.context().sources().isEmpty()){var s=r.context().sources().getFirst();String quote=s.exactEvidence()==null||s.exactEvidence().isEmpty()?firstSentence(s.contentExcerpt()):s.exactEvidence().getFirst();if(QuestionProposalValidator.normalize(quote).contains(QuestionProposalValidator.normalize(actor)))evidence.add(new SourceEvidence(s.sourceId(),s.sourceSectionId(),s.title(),s.checksum(),quote));}
