...
final class LessonPageClaimValidator {
  ...
  private static final Pattern TRANSITION=Pattern.compile("(?iu)^(i den här lektionen|på den här sidan|kom ihåg|sammanfattningsvis|läs vidare)\\b");
  ...
  Result validate(LessonGenerationProviderClient.Page page,String source,List<String> facts){
    ...
    for(String raw:page.body().split("(?<=[.!?])\\s+")){
      ...
      if(!seen.add(normalized)){code="DUPLICATE_CLAIM";diagnostic="The same substantive claim already occurs on this page.";}
  ...
  private String normalize(String value){return Normalizer.normalize(value==null?"":value,Normalizer.Form.NFKC).toLowerCase(Locale.ROOT).replace('\u00a0',' ').replaceAll("[^\\p{L}\\p{N}]+"," ").replaceAll("\\s+"," ").trim();}
}
