...
final class QuestionProposalValidator {
  ...
  void validateProposal(QuestionGenerationProviderClient.Request request,QuestionGenerationProviderClient.Proposal p){
    ...
    validateNarrowTarget(request,p);
  ...
  private void validateNarrowTarget(QuestionGenerationProviderClient.Request request,QuestionGenerationProviderClient.Proposal proposal){var target=request.narrowTarget();if(target==null)return;if(!proposal.questionType().equals(target.allowedQuestionForm()))fail("AI_QUESTION_EXPANSION_TARGET_MISMATCH","Question form differs from the immutable expansion target");if(target.exactEvidence()==null||request.context().sources().stream().noneMatch(source->source.exactEvidence()!=null&&source.exactEvidence().stream().anyMatch(evidence->normalizeEvidence(evidence).equals(normalizeEvidence(target.exactEvidence())))))fail("AI_QUESTION_EXPANSION_TARGET_MISMATCH","Expansion target evidence is not part of the immutable Source snapshot");String answer=proposal.answerOptions()==null?"":proposal.answerOptions().stream().filter(QuestionGenerationProviderClient.Option::correct).map(QuestionGenerationProviderClient.Option::text).reduce("",(a,b)->a+" "+b);if(!plausiblyRelated(proposal.questionText()+" "+answer,target.testedProposition())||!plausiblyRelated(answer+" "+proposal.explanation(),target.correctAnswerBoundary()))fail("AI_QUESTION_EXPANSION_TARGET_MISMATCH","Question does not test the immutable planned proposition");String proposition=normalize(target.testedProposition()),boundary=normalize(target.correctAnswerBoundary());if(proposition.contains("tiden eller tidsperioden")&&!Pattern.compile("\\b(18|19|20)\\d{2}\\b|\\b(vart|varje|sedan|mellan|år|månad|dag|vecka|tid|period)\\b").matcher(boundary).find())fail("AI_QUESTION_EXPANSION_TARGET_INVALID","Chronology target has no explicit time boundary");}
