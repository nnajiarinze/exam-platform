...
@Component
final class GoogleBrokerOperationalProbe {
    ...
    GoogleBrokerOperationalProbe() { this(Clock.systemUTC()); }

    GoogleBrokerOperationalProbe(Clock clock) { this.clock = clock; }
        ...
        try {
            ...
            for (int hop = 0; hop < MAX_HOPS; hop++) {
                ...
                if (status >= 300 && status < 400) {
                    ...
                    if (isGoogleAuthorizationHost(host, next.getPath())) {
                        ...
                        return new ProbeResult(true, "GOOGLE_REDIRECT_REACHED", OffsetDateTime.ofInstant(now, ZoneOffset.UTC),
                                host, fingerprint(clientId), callbackMatches);
                    }
                    current = next;
                    continue;
                }
            ...
            return ProbeResult.degraded("BROKER_PROBE_FAILED", now);
        }
    }
    ...
    private static boolean isGoogleAuthorizationHost(String host, String path) {
        if ("accounts.google.com".equals(host) || "oauth2.googleapis.com".equals(host)) {
            return true;
        }
        return host.endsWith(".google.com") && path != null && path.contains("oauth2");
    }
