...
SELECT DISTINCT ON (old_progress.learner_id,new_release.id,new_topic.id)
  gen_random_uuid(),old_progress.learner_id,new_release.id,new_topic.id,first_section.id,
...
JOIN imported_content_release old_release ON old_release.exam_id=new_release.exam_id
  AND old_release.published_at<new_release.published_at
JOIN imported_topic old_topic ON old_topic.content_release_id=old_release.id
...
ORDER BY old_progress.learner_id,new_release.id,new_topic.id,old_progress.completed_at DESC
