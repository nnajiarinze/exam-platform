...
public class KnowledgeBaseService {
    ...
    public KnowledgeBaseService(JdbcClient jdbc,ReviewWorkflowService reviews,KnowledgeFactTextQualityValidator textQuality) { this.jdbc = jdbc; this.reviews=reviews; this.textQuality=textQuality; }
    ...
    @Transactional public Map<String,Object> submit(UUID id,ActionInput in) { expect(in.version()); var fact=fact(id); if(!List.of("UNREVIEWED","REJECTED","REQUIRES_UPDATE").contains(fact.get("reviewStatus"))||"RETIRED".equals(fact.get("status"))) throw conflict("Only an editable draft can be submitted"); var quality=textQuality.validate(String.valueOf(fact.get("canonicalStatement"))); if(quality.quality()!=KnowledgeFactTextQualityValidator.Quality.VALID){LOG.warn("knowledge_fact_submission_blocked factId={} actor={} validatorVersion={} quality={} issues={}",id,actor(),quality.diagnostics().validatorVersion(),quality.quality(),quality.issues().stream().map(KnowledgeFactTextQualityValidator.Issue::code).toList());throw textInvalid("Enter a meaningful, declarative factual statement before submission",quality);} if(sourceIds((UUID)fact.get("currentVersionId")).isEmpty()) throw validation("At least one source is required before submission");LOG.info("knowledge_fact_revalidated factId={} actor={} validatorVersion={} constructionType={} predicateForm={}",id,actor(),quality.diagnostics().validatorVersion(),quality.diagnostics().constructionType(),quality.diagnostics().predicateForm());boolean resubmit=List.of("REJECTED","REQUIRES_UPDATE").contains(fact.get("reviewStatus"));var result=transition(id,in.version(),"UNDER_REVIEW",null,null,"knowledge_fact.submitted");reviews.submitted("KNOWLEDGE_FACT",id,(UUID)fact.get("currentVersionId"),(String)version((UUID)fact.get("currentVersionId")).get("authorId"),(String)result.get("status"),resubmit);return result; }
    ...
    private void validateFact(FactInput in){if(in.learningObjectiveId()==null)throw validation("learningObjectiveId is required");require(in.canonicalStatement(),"canonicalStatement");var quality=textQuality.validate(in.canonicalStatement());if(quality.blocksDraftSave())throw textInvalid(quality.issues().getFirst().message(),quality);if(in.validFrom()!=null&&in.validTo()!=null&&in.validTo().isBefore(in.validFrom()))throw validation("validTo cannot be before validFrom");}
    private DomainException textInvalid(String message,KnowledgeFactTextQualityValidator.Result result){return new DomainException(HttpStatus.UNPROCESSABLE_ENTITY,"KNOWLEDGE_FACT_TEXT_INVALID",message,result.issues().stream().map(issue->(Object)Map.of("code",issue.code(),"message",issue.message(),"quality",result.quality().name(),"recommendedAction","EDIT_FACT")).toList());}
