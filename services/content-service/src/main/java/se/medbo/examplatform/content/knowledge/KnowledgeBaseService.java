...
public class KnowledgeBaseService {
    ...
    @Transactional public Map<String,Object> submit(UUID id,ActionInput in) { expect(in.version()); var fact=fact(id); if(!List.of("UNREVIEWED","REJECTED","REQUIRES_UPDATE").contains(fact.get("reviewStatus"))||"RETIRED".equals(fact.get("status"))) throw conflict("Only an editable draft can be submitted"); var quality=textQuality.validate(String.valueOf(fact.get("canonicalStatement"))); if(quality.quality()!=KnowledgeFactTextQualityValidator.Quality.VALID){LOG.warn("knowledge_fact_submission_blocked factId={} actor={} quality={} issues={}",id,actor(),quality.quality(),quality.issues().stream().map(KnowledgeFactTextQualityValidator.Issue::code).toList());throw textInvalid("Enter a meaningful, declarative factual statement before submission",quality);} if(sourceIds((UUID)fact.get("currentVersionId")).isEmpty()) throw validation("At least one source is required before submission");boolean resubmit=List.of("REJECTED","REQUIRES_UPDATE").contains(fact.get("reviewStatus"));var result=transition(id,in.version(),"UNDER_REVIEW",null,null,"knowledge_fact.submitted");reviews.submitted("KNOWLEDGE_FACT",id,(UUID)fact.get("currentVersionId"),(String)version((UUID)fact.get("currentVersionId")).get("authorId"),(String)result.get("status"),resubmit);return result; }
