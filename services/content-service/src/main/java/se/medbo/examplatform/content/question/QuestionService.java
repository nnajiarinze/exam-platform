...
public class QuestionService {
    ...
    @Transactional public Map<String,Object> submit(UUID id,ActionInput in) { expect(in.version()); var q=question(id); if(!List.of("UNREVIEWED","REJECTED","REQUIRES_UPDATE").contains(q.get("reviewStatus"))||"RETIRED".equals(q.get("status"))) throw conflict("Only an editable draft can be submitted"); validateCurrent(q);boolean resubmit=List.of("REJECTED","REQUIRES_UPDATE").contains(q.get("reviewStatus"));var result=transition(id,in.version(),"UNDER_REVIEW",null,null,"question.submitted");reviews.submitted("QUESTION",id,(UUID)q.get("currentVersionId"),(String)q.get("authorId"),(String)result.get("status"),resubmit);return result; }
    @Transactional public Map<String,Object> approve(UUID id,ActionInput in) { expect(in.version()); var q=question(id); requireReview(q,"UNDER_REVIEW"); if(actor().equals(q.get("authorId"))) throw new DomainException(HttpStatus.FORBIDDEN,"SEPARATION_OF_DUTIES_REQUIRED","An author cannot approve their own question"); validateCurrent(q);var result=transition(id,in.version(),"APPROVED","ACTIVE",in.reason(),"question.approved");reviews.decision("QUESTION",(UUID)q.get("currentVersionId"),"UNDER_REVIEW","APPROVED","ACTIVE",in.reason(),in.reasonCode());return result; }
    @Transactional public Map<String,Object> reject(UUID id,ActionInput in) { expect(in.version());reviews.validateFeedback("REJECTED",in.reasonCode(),in.reason());var q=question(id);requireReview(q,"UNDER_REVIEW");var result=transition(id,in.version(),"REJECTED","DRAFT",in.reason(),"question.rejected");reviews.decision("QUESTION",(UUID)q.get("currentVersionId"),"UNDER_REVIEW","REJECTED","DRAFT",in.reason(),in.reasonCode());return result; }
    @Transactional public Map<String,Object> requireUpdate(UUID id,ActionInput in) { expect(in.version());reviews.validateFeedback("REQUIRES_UPDATE",in.reasonCode(),in.reason());var q=question(id);requireReview(q,"UNDER_REVIEW");var result=transition(id,in.version(),"REQUIRES_UPDATE","DRAFT",in.reason(),"question.requires_update");reviews.decision("QUESTION",(UUID)q.get("currentVersionId"),"UNDER_REVIEW","REQUIRES_UPDATE","DRAFT",in.reason(),in.reasonCode());return result; }
    ...
    private void validateCurrent(Map<String,Object> q) { UUID versionId=(UUID)q.get("currentVersionId");if(factIds(versionId).isEmpty())throw validation("At least one approved knowledge fact is required");long invalid=jdbc.sql("SELECT count(*) FROM question_knowledge_fact qkf JOIN knowledge_fact_version kfv ON kfv.id=qkf.knowledge_fact_version_id JOIN knowledge_fact f ON f.id=kfv.knowledge_fact_id WHERE qkf.question_version_id=:id AND (f.status<>'ACTIVE' OR f.review_status<>'APPROVED' OR f.current_version_id<>kfv.id)").param("id",versionId).query(Long.class).single();if(invalid>0)throw validation("Linked knowledge facts must still be approved and active"); }
