...
class QuestionValidatorTest {
    ...
    @Test void acceptsMultipleChoiceWithMultipleCorrectOptions(){assertThatCode(()->validator.validate(input("MULTIPLE_CHOICE",List.of(option(0,"A",true),option(1,"B",false),option(2,"C",true)),null))).doesNotThrowAnyException();}
    @Test void rejectsMultipleChoiceWithoutACorrectOption(){assertThatThrownBy(()->validator.validate(input("MULTIPLE_CHOICE",List.of(option(0,"A",false),option(1,"B",false)),null))).isInstanceOf(DomainException.class).hasMessageContaining("at least one");}
