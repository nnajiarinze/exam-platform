...
class ContentServiceIntegrationTest {
    ...
    void authorCanCreateListAndUpdateExamWithOptimisticLocking() throws Exception {
        var body = """
                {"code":"SWEDISH_CITIZENSHIP","name":"Swedish Citizenship","countryCode":"SE","status":"DRAFT"}
                """;
        ...
        mvc.perform(get("/api/v1/admin/exams").queryParam("search", "SWEDISH_CITIZENSHIP").headers(author())).andExpect(status().isOk()).andExpect(jsonPath("$.totalItems").value(1));
        var update = """
                {"code":"SWEDISH_CITIZENSHIP","name":"Updated","countryCode":"SE","status":"DRAFT","version":0}
                """;
    ...
    void questionEndpointCreatesValidatesReviewsSearchesAndVersionsApprovedQuestions() throws Exception {
        ...
        jdbc.sql("INSERT INTO exam(id,code,name,country_code,status,created_at,updated_at) VALUES(:id,:code,'Questions','SE','DRAFT',:now,:now)").param("id",exam).param("code","QUESTIONS_"+exam).param("now",now).update();
        jdbc.sql("INSERT INTO exam_version(id,exam_id,version_code,display_name,status,created_at,updated_at) VALUES(:id,:exam,'V1','V1','DRAFT',:now,:now)").param("id",examVersion).param("exam",exam).param("now",now).update();
